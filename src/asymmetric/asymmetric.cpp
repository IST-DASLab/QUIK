...
torch::Tensor int4FusedDequantize(const torch::Tensor &A,
  ...
  torch::checkDeviceType("int4FusedDequantize", {A, B, scale_row, scale_col, zero_row, w_reduced},
                         at::DeviceType::CUDA);
  return int4FusedDequantizeCUDA(A, B, scale_row, scale_col, shift_value,  zero_row, w_reduced);
}
...
torch::Tensor int8FusedDequantize(const torch::Tensor &A,
  ...
  torch::checkAllContiguous("int8FusedDequantize", {{A, "A", 0},
                                                    ...
                                                    {zero_row, "zero_row", 4},
                                                    {w_reduced, "w_reduced", 5}});
  torch::checkDeviceType("int8FusedDequantize", {A, B, scale_row, scale_col, zero_row, w_reduced},
                         at::DeviceType::CUDA);
  return int8FusedDequantizeCUDA(A, B, scale_row, scale_col, shift_value,  zero_row, w_reduced);
}
...
torch::Tensor int4SpFusedDequantize(const torch::Tensor &A,
  ...
  torch::checkDeviceType("int4SpFusedDequantize",
                         {A, B, scale_row, scale_col, zero_row, w_reduced}, at::DeviceType::CUDA);
  return int4SpFusedDequantizeCUDA(A, B, E, scale_row, scale_col, shift_value, zero_row, w_reduced);
}
...
torch::Tensor int8SpFusedDequantize(const torch::Tensor &A,
  ...
  torch::checkDeviceType("int4SpFusedDequantize",
                         {A, B, scale_row, scale_col, zero_row, w_reduced}, at::DeviceType::CUDA);
  return int8SpFusedDequantizeCUDA(A, B, E, scale_row, scale_col, shift_value, zero_row, w_reduced);
}
...
void buildSubmodule(py::module &mod) {
  ...
  m.def("int4FusedDequantize", &int4FusedDequantize,
      ...
      "N, FP16, CUDA)\n"
      "shift_value: torch.Tensor(1 x 1, FP16).item())"
      "zero_row: torch.Tensor(M x 1, FP16, CUDA)"
      "w_reduced: torch.Tensor(1 x N, FP16, CUDA)"
      "output: torch.Tensor(M x N, INT32, CUDA)\n"
      "output = int4Unpacking(A) @ int4Unpacking(B)^T * scale_cal * scale_row +(zero_row + shift_value * scale_row) * w_reduced",
  ...
  m.def(
      ...
      "w_reduced: torch.Tensor(1 x N, FP16, CUDA)"
      "output: torch.Tensor(M x N, INT32, CUDA)\n"
      "output = A @ B^T * scale_cal * scale_row +(zero_row + shift_value * scale_row) * w_reduced",
      py::arg("A"), py::arg("B"), py::arg("E"), py::arg("scale_row"),
      py::arg("scale_col"), py::arg("shift_value"), py::arg("zero_row"), py::arg("w_reduced"));
