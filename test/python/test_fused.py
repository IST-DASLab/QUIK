...
def int4_asy_fused_test():
    ...
    scale_row = torch.rand(M, 1, dtype=torch.float16).cuda()
    scale_col = torch.rand(1, N, dtype=torch.float16).cuda()
    shift_value = torch.rand(1, 1, dtype=torch.float16)
    ...
    c_ref = c_ref * scale_col * scale_row + (zero_row + shift_value.item() * scale_row) * w_reduce

    c = quik.asymmetric.int4FusedDequantize(a, b, scale_row, scale_col, shift_value.item(), zero_row, w_reduce)
...
def int4_sparse_fused_test():
    ...
    c_ref = quik.symmetric.int4FusedDequantize(qa_uncompressed.cuda(), b, scale_row, scale_col, y)
...
def int4_asy_sparse_fused_test():
    ...
    c = quik.asymmetric.int4SpFusedDequantize(a, b, e, scale_row, scale_col, shift_value.item(), zero_row, w_reduce)

    qa_uncompressed = quik.matmul.int4Uncompress(a.cpu(), metadata, M, K)
    c_ref = quik.asymmetric.int4FusedDequantize(qa_uncompressed.cuda(), b, scale_row, scale_col, shift_value.item(), zero_row, w_reduce)
...
def int8_asy_fused_test():
    ...
    c = quik.asymmetric.int8FusedDequantize(a, b, scale_row, scale_col, shift_value.item(), zero_row, w_reduce)
    assert (torch.equal(c, c_ref))
...
def int8_sparse_fused_test():
    ...
    c_ref = quik.symmetric.int8FusedDequantize(qa_uncompressed.cuda(), b, scale_row, scale_col, y)
...
def int8_asy_sparse_fused_test():
    ...
    c = quik.asymmetric.int8SpFusedDequantize(a, b, e, scale_row, scale_col, shift_value.item(), zero_row, w_reduce)

    qa_uncompressed = quik.matmul.int8Uncompress(a.cpu(), metadata, M, K)
    c_ref = quik.asymmetric.int8FusedDequantize(qa_uncompressed.cuda(), b, scale_row, scale_col, shift_value.item(), zero_row, w_reduce)
